#include <stdio.h>
#include "../tabb/tabb.h"
int TABB_conta_nummaior(TABB*a, int x) {
    if (!a) return 0;
    if (a->info > x) return 1 + TABB_conta_nummaior(a->esq, x) + TABB_conta_nummaior(a->dir, x);
    return TABB_conta_nummaior(a->dir, x);
}
...
int* mN(TABB*a, int N) {
    if (!a) return NULL;
    int cont = TABB_conta_nummaior(a, N);
